            ...
            <div class="card-body">
                <table class="table table-hover table-sm">
                    <caption>
                        <div class="row">
                            <div class="col-md-9">
                                List of Types - Total Items {{ this.meta.total }}
                            </div>
                            <div class="col-md-3">
                    ...
                    <tbody v-if="types">
                        <tr v-for="{ id, name, description } in types">
                            <td>{{ id }}</td>
                            <td>{{ name }}</td>
                            <td>{{ description }}</td>
                            <td>
                                <router-link class="text-info" :to="{ name: 'types.view', params: { id: id }}">View</router-link>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<script>
const getTypes = (page, per_page, callback) => {
...
export default {
    ...
    beforeRouteEnter (to, from, next) {
        if (to.query.per_page == null) {
            getTypes(to.query.page, 10, (err, data) => {
                next(vm => vm.setData(err, data));
            });
        } else {
            getTypes(to.query.page, to.query.per_page, (err, data) => {
                next(vm => vm.setData(err, data));
            });
        }
    },
    ...
    beforeRouteUpdate (to, from, next) {
        getTypes(to.query.page, this.meta.per_page, (err, data) => {
    ...
    methods: {
        ...
        setData(err, { data: types, links, meta }) {
            this.pageNumbers = [];
